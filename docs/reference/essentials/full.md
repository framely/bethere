# Cost-effective conversational services

To run a business using conversational applications like chatbots or agents, simply reacting to user requests is not enough. Your agent (or chatbot) must proactively guide the conversation toward outcomes that benefit your business as well. To achieve this under continuously changing business conditions and in a differentiated manner, you need to be able to design and program your agent. 

Flowcharts have been used to design user experiences for applications with graphical user interfaces (GUIs), and now this technique is also widely adopted in building chatbots—almost all chatbot platforms are flow-based. However, despite the availability of many such platforms in the market, and the fact that there are apps for just about anything, usable conversational experiences are still few and far between. Why?
