...
Building a usable conversational user interface used to be difficult and expensive. However, with recent advances in [language models for natural language understanding](../reference/essentials/du/chatgpt-reset.md) and new [type-based interaction logic design](../reference/essentials/index.md), particularly the [dual-process approach based on retrieval augmented generation](../reference/essentials/dual-process.md), usability will significantly improve while costs will drastically decrease. Since you don’t have to worry about different screen sizes and natural language-based responses require no learning curve at all, building a copilot can be a more cost-effective way to expose functionality.
